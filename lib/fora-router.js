(function () {
    ...
    Router.prototype.doRouting = function*(request, next) {
        for(let i = 0; i < this.onRequestHandlers.length; i++) {
            _ = yield* this.onRequestHandlers[i].call(this, next);
        }
                    ...
                    case "pattern":
                        if (route.method === request.method) {
                            ...
                            if (m) {
                                ...
                                _ = yield* route.handler.apply(request, args);
                                return next ? (yield next) : void 0;
                            }
                        }
                        break;
    ...
    Router.prototype.koaRoute = function() {
        ...
        return function*(next) {
            ...
            _ = yield* self.doRouting(request, next);
    ...
    Router.prototype.route = function() {
        var self = this;
        return function*(next) {
            _ = yield* self.doRouting(this, next);
        };
    };
